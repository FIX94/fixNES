...
inline int vrc7FromDb(double in)
{
	return (int)(in * attenDb);
}
...
static int vrc7LinearLut[65536];
static int vrc7KslLut[0x10];
...
void vrc7AudioInit()
{
	...
	for(i = 0; i < 65536; ++i)
	{
		int outscale = (1 << 20);  // channel output is 20 bits wide
		double inscale = attenDb / (1 << 7);
		double lin = pow(10.0, (i / -20.0 / inscale));
		vrc7LinearLut[i] = (int)(lin * outscale);
	}
...
void vrc7SetR(vrc7Ksr_t *k, int r)
{
	r = (r * 4) + k->K;
	k->RH = r>>2;
	if(k->RH > 15)
...
static void vrc7CalcSlotVals(vrc7chan_t *chan, vrc7slot_t *slot, uint8_t slotNum)
{
    ...
    // Ksl
    int kslbits = vrc7instrument[chan->instrument][2 | slotNum] >> 6;
    if(kslbits)
    {
        int a = vrc7KslLut[chan->freq >> 5] - vrc7FromDb(6)*(7 - chan->block);
        if(a <= 0)
            slot->kslAtten = 0;
        else
...
static int vrc7GetOut(vrc7chan_t *chan, vrc7slot_t *slot, uint8_t slotNum, uint32_t inV)
{
    ...
    int output = vrc7LinearLut[env>>7];
	int Rectify = (vrc7instrument[chan->instrument][3] & (0x8 << slotNum)) ? 0 : -1;
    if(inV & (1<<17))
        output *= Rectify;
	slot->out = output;
...
void vrc7AudioCycle()
{
	...
	for(i = 0; i < 6; i++)
	{
		...
		int carryOut = vrc7GetOut(c, &c->carry, 1, modOut);
		vrc7Out += carryOut;
	}
}
